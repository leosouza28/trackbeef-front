...
export class ProdutosListarComponent {
  ...
  async busca(q: any) {
    ...
    try {
      ...
      let data: any = await this.endpointsService.getProdutos({ ...values });
      this.data = data;
    } catch (error) {
  ...
  open($event: any, item: any) {
    if ($event?.ctrlKey || $event?.metaKey) {
      window.open(`/admin/produtos/form?id=${item._id}`, '_blank');
    } else {
      this.router.navigate([`/admin/produtos/form`], { queryParams: { id: item._id } });
    }
